...
#         pygame.draw.rect(screen, colorYELLOW, (segment.x * CELL, segment.y * CELL, CELL, CELL))  # Рисуем тело


# def draw(self):
#     head = self.body[0]
#     pygame.draw.circle(screen, colorRED, 
#                        (head.x * CELL + CELL // 2, head.y * CELL + CELL // 2),  
#                        CELL // 2)  # Рисуем голову как круг
...
# def reset_game():
#     global snake, food  # Используем глобальные переменные
...
ENEMY_SPEED = 4
coin_count = 0
crash_count = 0  # Новый счетчик аварий
...
while running:
    ...
    crash_text = score_font.render(f"Crashes: {crash_count}", True, "white")
    screen.blit(crash_text, (10, 10))
